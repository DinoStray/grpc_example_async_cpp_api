...
bool GreetingSession::init() {
    GreetingServer::getInstance().greeting_async_service_.RequestSubscribeGreetingBySecond(
        ...
        reinterpret_cast<void*>(session_id_ + GRPC_EVENT_CONNECTED));
    return true;
}
...
void GreetingSession::process(GrpcEvent event) {
    ...
    switch (event) {
        case GRPC_EVENT_CONNECTED:
            subscribe_stream.Read(&request_, reinterpret_cast<void*>(session_id_ + GRPC_EVENT_READ_DONE));
        ...
        case GRPC_EVENT_WRITE_DONE:
            if (!message_queue_.empty()) {
                ...
                subscribe_stream.Write(*message_queue_.front(),
                                       reinterpret_cast<void*>(session_id_ + GRPC_EVENT_WRITE_DONE));
...
void GreetingSession::reply() {
    ...
    if (status_ == SessionStatus::READY_TO_WRITE) {
        status_ = SessionStatus::WAIT_WRITE_DONE;
        subscribe_stream.Write(*new_message, reinterpret_cast<void*>(session_id_ + GRPC_EVENT_WRITE_DONE));
    } else {
